...
export const SliderItem = ({itemActive, id, brand, name, image, desc,}: SliderItemProps) => {
    return(
        <li className={cn(
            ...
            <div className="space-y-4 absolute left-[10%] top-[20%] w-[500px] max-w-[80%] z-10">
                ...
                <h2 className={
                    cn("text-6xl lg:text-8xl m-0 text-white font-bold translate-y-[30px] blur-[20px] opacity-0 animation-delay-4", itemActive === id && "animate-show-content")
                }>{name}</h2>
                <p className={cn("text-gray-200 translate-y-[30px] blur-[20px] opacity-0 animation-delay-5", itemActive === id && "animate-show-content")}>{desc}</p>
            </div>
        </li>
    )
}
